def test_catalog_combines_all_resource_types(client):
    ...
    client.post(
        "/v1/mcp-servers",
        json={"name": "m1", "namespace": "kagent", "url": "http://x", "protocol": "SSE"},
    )
...
def test_catalog_q_filter(client):
    ...
    client.post(
        "/v1/agents", json={"name": "other-agent", "namespace": "kagent", "system_message": "hi"}
    )
...
def test_catalog_q_filter_matches_mcp_discovered_tool(client, fake_k8s):
    ...
    fake_k8s.store[key]["status"]["discoveredTools"] = [
